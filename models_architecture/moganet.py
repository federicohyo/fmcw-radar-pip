...
def custom_moganet_pretrained(input_channels = 1, num_classes = 11):
    # Initialize model with pretrained weights
    model = moganet_tiny(pretrained=True, drop_rate=0.2)

    # Modify the initial layer
    # Assuming the initial conv layer is named 'patch_embed1.projection'
    initial_in_channels = model.patch_embed1.projection[0].in_channels
    initial_out_channels = model.patch_embed1.projection[0].out_channels
    initial_kernel_size = model.patch_embed1.projection[0].kernel_size
    initial_stride = model.patch_embed1.projection[0].stride
    initial_padding = model.patch_embed1.projection[0].padding

    model.patch_embed1.projection[0] = nn.Conv2d(input_channels, initial_out_channels, kernel_size=initial_kernel_size, stride=initial_stride, padding=initial_padding)

    # Modify the final layer
    model.head = nn.Linear(model.embed_dims[-1], num_classes)
